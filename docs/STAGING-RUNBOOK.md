...
3. Kiểm tra env readiness sau khi đã export biến môi trường hoặc nạp env theo quy trình local/staging:

   ```bash
   pnpm check:env
   ```

   Checker chỉ báo `OK`, `MISSING` hoặc `INVALID`. Checker không in `DATABASE_URL`, `SYNC_SECRET`, SQL password, API key hoặc connection string. Lệnh này không connect MySQL, không connect SQL Server, không gọi API server và không chạy sync.

   Nếu muốn kiểm tra riêng `IMAGE_BASE_PATH` có tồn tại hay không, bật cờ:

   ```bash
   CHECK_IMAGE_BASE_PATH_EXISTS=true pnpm check:env
   ```

4. Chuẩn bị MySQL test/staging.

5. Rà soát `.env` thủ công, bảo đảm không dùng secret production nếu chưa được phép.

6. Đọc thêm checklist migration/rollback trong `docs/DATABASE-RUNBOOK.md`.

7. Chỉ chạy migration khi đã có xác nhận riêng:
...
8. Chạy server local/staging.

9. Chạy client local.

10. Test lookup với dữ liệu mẫu trước, xác nhận API public không trả SoCMT gốc hoặc field nội bộ.

11. Test upload ảnh mẫu bằng JPG nhỏ, không dùng ảnh học viên thật trong repo.

12. Test sync-tool với batch nhỏ 1-5 record trước.

13. Kiểm tra checkpoint:
